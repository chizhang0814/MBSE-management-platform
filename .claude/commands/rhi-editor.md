...
以下是用户在开发过程中逐步确认的设计决策，下次恢复开发时必须遵守：

### 交互方式
1. **无模式切换**：取消了连线模式/浏览模式切换按钮，所有操作通过鼠标拖拽自然完成
2. **拖拽框体=移动节点**：鼠标按住设备框/连接器区域或互联点框体（非针孔区域）可拖动位置
3. **拖拽针孔=连线**：鼠标按住任意节点的针孔区域，拖到另一个节点即创建连线
4. **禁止设备间直连**：连线必须有互联点参与，设备拖向设备不创建连线
5. **右键删除互联点**：右键点击互联点即可从画布移除，恢复左侧面板可拖状态
6. **点击连线删除**：点击已有连线弹出确认后删除

### 互联点管理
7. **左侧面板拖入画布**：不再用按钮添加，从左侧面板拖拽互联点到画布区域
8. **拖入弹出针孔分配对话框**：松开后弹窗让用户为每个协议标识选择对应针孔
9. **已占用针孔禁用**：被其他信号组占用的针孔在下拉菜单中禁用并显示"已被XXX占用"
10. **可用针孔排前面**：下拉菜单中可用针孔排在已占用之前
11. **已在画布的互联点变灰**：不可重复拖入，右键移除后恢复
12. **左侧面板分组显示**：默认按类型分组，可切换按区域或不分组
13. **互联点有类型和区域属性**：ic_type 和 ic_zone，在管理弹窗中创建/编辑

### 视觉设计
14. **设备节点**：设备框(teal) + 连接器竖条(深灰) + 针孔列表(每个两行：针孔号+协议标识)
15. **互联点节点**：橙色实线框（不是虚线）、右上角RHI圆角矩形徽章、标签和类型·区域居中上方、针孔在框内
16. **不要汇聚点**：取消了设备节点的中心汇聚圆点
17. **针孔附着点在针孔矩形边缘**：不是在互联点大框边缘，左右各一个小圆点
18. **连线穿入框内**：连线图层在节点之上渲染（SVG后绘制），穿过互联点外框直达内部针孔

### 连线渲染
19. **针孔到针孔**：连线从设备针孔右端直接到互联点针孔左/右端（自动判断方向）
20. **绞线从各自针孔出发**：起始5%从各自针孔位置渐变汇入基线，中间90%绞线交叉，终止5%分开进入各自针孔
21. **绞线统一配色**：双绞=红+蓝，三绞=红+蓝+绿，所有绞线组相同配色（不按协议标识分色）
22. **单线用协议标识颜色**：非绞线信号用 COLOR_POOL 轮换色

### 布局
23. **互联点居中**：初始布局互联点在画布中心竖向排列，设备环绕在外圈

### RHI按钮
24. **灰色=待构建**：未添加互联点或连线的信号组
25. **橙色=已构建**：已保存互联点或连线的信号组
26. **当前已隐藏**：display:none 临时隐藏，恢复时删除该行即可（代码位置有 TODO 注释）

## 当前状态

- RHI 按钮和互联点管理按钮已用 `display: 'none'` 临时隐藏
- 恢复方法：搜索 `TODO: 恢复RHI功能时去掉此行`，删除对应的 `display: 'none'` 行
- 涉及文件：`client/src/pages/ProjectDataView.tsx`（两处）

## 下一阶段 TODO

- [ ] 互联点节点在画布中双击或右键编辑已分配的针孔（重新弹出分配对话框）
- [ ] 保存后重新加载时保留节点位置（当前每次加载都重新计算布局）
- [ ] 连线删除后自动刷新 RHI 状态按钮颜色
- [ ] 导出 CSV 时包含互联点针孔号（从 interconnect_pins 获取）
- [ ] 画布缩放（Ctrl+滚轮）和平移（空格+拖拽）
- [ ] 恢复 RHI 按钮和互联点管理按钮的可见性
- [ ] 在生产环境验证完整流程（创建互联点→拖入画布→分配针孔→连线→保存→导出）

## 相关文件

```
client/src/components/RhiEditor.tsx   RHI编辑器React组件
client/src/pages/ProjectDataView.tsx  互联点管理弹窗 + RHI按钮(已隐藏) + RHI弹窗入口
server/src/routes/rhi.ts             RHI编辑器API（status/all在/:signal_group之前）
server/src/routes/interconnects.ts   互联点CRUD + Excel导入API（含pin占用查询）
server/src/database.ts               5张表建表迁移 + 旧表FK修复迁移 + edge去重+UNIQUE约束
