...
import { authenticate, requireRole, requireAdminOrZontiOrSystem, AuthRequest } from '../middleware/auth.js';
import fs from 'fs';
...
export function uploadRoutes(db: Database) {
  ...
  // 获取上传文件列表
  router.get('/files', authenticate, requireAdminOrZontiOrSystem(db), async (req, res) => {
    try {
  ...
  // 获取单个文件详情
  router.get('/files/:id', authenticate, requireAdminOrZontiOrSystem(db), async (req, res) => {
    try {
  ...
  // 下载已上传文件
  router.get('/files/:id/download', authenticate, requireAdminOrZontiOrSystem(db), async (req, res) => {
    try {
